...
BEGIN
    ...
    FROM (
	    SELECT letter, SUM(oc) total, COUNT(*) occurrence, MAX(oc) max_occurrence  
        FROM (SELECT letter, str, CHAR_LENGTH(str) - CHAR_LENGTH(REPLACE(str, letter, '')) oc FROM strs, l) e
