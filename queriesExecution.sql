...
BEGIN
    ...
    SET i = 0;
    SET @n = (SELECT COUNT(*) FROM queries);

    CREATE TEMPORARY TABLE results (query_name TEXT, val TEXT);

    WHILE i < @n DO
        SET @d = CONCAT(
            'INSERT INTO results SELECT \'',
            (SELECT query_name FROM queries LIMIT i, 1),
            '\', (',
            (SELECT code FROM queries LIMIT i, 1),
            ')'
        );
